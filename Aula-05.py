...
def main():
    ...
    if choice == 'EDA':
        ...
        if st.sidebar.checkbox("Adding Interactive Filters"):
            houses = st.dataframe(df[['id', 'lat', 'long', 'price', 'level', 'is_waterfront']])
        ...
        if st.sidebar.checkbox("Map for Number Bathrooms"):
            #is_check = st.sidebar.checkbox('Display Map for Price')
            houses = df[['id', 'lat', 'long', 'price', 'level','bathrooms']].copy()
            bathrooms_min = int(df['bathrooms'].min())
            bathrooms_max = int(df['bathrooms'].max())
            bathrooms_avg = int(df['bathrooms'].mean())

            bathrooms_slider = st.sidebar.slider('Bathrooms range',
                                             bathrooms_min,
                                             bathrooms_max,
                                             bathrooms_avg)

            houses = df[df['bathrooms'] < bathrooms_slider][['id', 'lat', 'long', 'price','level','bathrooms']]
            #st.dataframe(houses)
            fig = px.scatter_mapbox(houses,
                                    lat='lat',
                                    lon='long',
